...
def main() -> None:
    ...
    apr_gammas = [
        0.85 if name in PURE_RL_FINANCIERS else 0.70
        for name in FINANCIER_NAMES
    ]
    apr_epsilons = [
    ...
    for capital in CAPITALS_TO_RUN:
        for default_scenario in DEFAULT_SCENARIOS_TO_RUN:
            ...
            print(
                f"Running simulation for capital = {capital:,.2f}, "
                f"default scenario = {scenario_name}..."
            )
            ...
            if SAVE_Q_TABLES:
                ...
                q_table_output_path = Q_TABLE_OUTPUT_DIR / (
                    f"trained_q_tables_{int(capital)}_{scenario_name}.json"
                )
            ...
            if WRITE_DETAILED_SIMULATION_EXCEL:
                excel_path = Path(
                    f"simulation_results_4000_{int(capital)}_{scenario_name}_no_wholesaler_reward_agent.xlsx"
                )
            ...
            all_reports.append(
                f"================================================================================\n"
                f"                CAPITAL: {capital:,.2f} | DEFAULT SCENARIO: {scenario_name}\n"
                f"================================================================================\n"
                f"{report}\n"
            )
