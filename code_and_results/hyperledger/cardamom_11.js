...
async submitFederatedQUpdate(ctx, roundId, qTableJson, modelVersion = '', metadataJson = '{}', finIdOrEmpty = '') {
  ...
  const parsed = JSON.parse(String(qTableJson || '{}'));
  const hasNamedTables = parsed.pure_rl_q || parsed.hybrid_rl_q || parsed.borrower_premium_q || parsed[round];
  const qTablesPayload = hasNamedTables ? parsed : { borrower_premium_q: parsed };
  const metadata = JSON.parse(String(metadataJson || '{}'));
  if (modelVersion || Object.keys(metadata).length) {
    qTablesPayload.metadata = { ...metadata, modelVersion: String(modelVersion || metadata.modelVersion || '') };
  }
  return this.submitFederatedQTableUpdate(ctx, JSON.stringify(qTablesPayload), finIdOrEmpty, round);
}

async aggregateFederatedQTables(ctx, roundId, tableName = 'borrower_premium_q') {
  const round = String(roundId || '').trim();
  if (!round) throw new Error('roundId required');
  return this.aggregateFinanceFederatedQTables(ctx, round, tableName);
}

async getFederatedQTable(ctx, roundId, tableName = 'borrower_premium_q') {
  const round = String(roundId || '').trim();
  if (!round) throw new Error('roundId required');
  return this.getAggregatedFederatedQTable(ctx, round, tableName);
}

async listFederatedQUpdatesForRound(ctx, roundId) {
  const round = String(roundId || '').trim();
  if (!round) throw new Error('roundId required');
  return this.listFederatedQTableUpdates(ctx, round);
}
